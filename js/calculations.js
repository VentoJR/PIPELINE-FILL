...
function calculateFulfillment(rows) {
  ...
  const inventarioTotalProyectado = sumField(rows, "inventarioTotalProyectado");
  ...
  // Qué tan avanzado va el pedido revisado respecto al total proyectado de la temporada.
  const pctAvancePipeline = safeDiv(pedidoRevisado, inventarioTotalProyectado);

  return { forecast, pedidoRevisado, inventarioCumplido, trasladoCompletado, inventarioTotalProyectado, pendiente, pctPedidoVsForecast, pctCumplimiento, pctAvancePipeline };
}
